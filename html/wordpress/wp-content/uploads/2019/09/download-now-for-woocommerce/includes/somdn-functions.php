...
function somdn_load_plugin_textdomain() {
	...
	$locale = apply_filters( 'plugin_locale', $locale, 'download-now-for-woocommerce' );

	// Filename of the .mo translation file for the user locale
	// Example: {text-domain}-es_CO.mo, Colombian Spanish
	$mofile = 'download-now-for-woocommerce' . '-' . $locale . '.mo';

	// Clear this plugin textdomain if it's loaded
	unload_textdomain( 'download-now-for-woocommerce' );
	...
	load_textdomain( 'download-now-for-woocommerce', WP_LANG_DIR . '/free-downloads/' . $mofile );

	// Try loading a translation from the default languages locations (starting with this plugin's languages folder $lang_dir)
	load_plugin_textdomain( 'download-now-for-woocommerce', false, $lang_dir );
...
function somdn_get_checkbox_error_text() {
	...
	if ( empty( $checkbox_error ) ) {
		$checkbox_error = __( 'Please select at least 1 checkbox', 'download-now-for-woocommerce' );
	} else {
...
function somdn_check_key_for_download() {
	return apply_filters( 'somdn_check_key_for_download', $check = true );
}
...
function somdn_verify_download_request_key( $query_arg = 'somdn_download_key' ) {
	...
	if ( empty( $key ) ) {
		$key_error_none = __( 'Error 1: Invalid download key. Please try again.', 'download-now-for-woocommerce' );
...
function somdn_verify_download_key( $key = '', $query_arg ) {
	...
	$somdn_download_key = somdn_decode_download_key( $key, true );

	if ( ! somdn_is_timestamp( $somdn_download_key ) ) {
		$key_error_timestamp = __( 'Error 2: Invalid download key. Please try again.', 'download-now-for-woocommerce' );
	...
	if ( ! somdn_is_key_valid( $somdn_download_key ) ) {
		$key_error_time = __( 'Your download key has expired. Please try again.', 'download-now-for-woocommerce' );
...
function somdn_output_download_count_output_above( $product_id ) {
	...
	if ( ! empty( $is_output ) && $is_output == 1 ) {
		...
		if ( empty( $output_text ) ) {
			$output_text = sprintf( __( '(Downloads - %s)', 'download-now-for-woocommerce' ), '{count}' ) ;
...
function somdn_get_forum_link_basic() {
	$text = __( 'If you need further support please visit the support forum for this plugin over at', 'download-now-for-woocommerce' );
...
function somdn_write_log( $log = '' ) {
	...
	$new_enty = $log_time . $log_timezone . ' ' . $log;
	$file_content = '';
